...
def main():
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if "train" in phase:
            ...
            print(f'Dataset {phase} created')
            train_idx = int(phase.split("_")[1])
            assert train_idx in [0, 1] and train_idx not in train_loaders
            ...
            if opt['dist']:
                ...
                train_sampler = None
            #
            assert train_idx not in train_loaders and train_idx not in train_samplers
            train_loader = create_dataloader(train_set, dataset_opt, opt, train_sampler)
            train_loaders[train_idx] = train_loader
            train_samplers[train_idx] = train_sampler
    ...
    assert len(train_loaders) == len(train_samplers) == 2

    #### create model
    current_step = 0 if resume_state is None else resume_state['iter']
    model_0 = create_model(opt, current_step)
    model_1 = create_model(opt, current_step)
    model_1.netG.module.RRDB = model_0.netG.module.RRDB  # shared
    models = {0: model_0, 1: model_1}

    # Load model weights
    if args.model_path is not None:
        assert resume_state is None
        state_dict = torch.load(args.model_path, map_location=model_0.device)
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        if opt['dist']:
            for _, train_sampler in train_samplers.items():
                train_sampler.set_epoch(epoch)

        print('epoch', epoch)

        for train_idx, train_loader in train_loaders.items():
            ...
            for _, train_data in enumerate(train_loader):
                ...
                if current_step <= total_iters:
                    ...
                    if current_step % opt['logger']['print_freq'] == 0 \
                        ...
                        ram = process.memory_info().rss / (2 ** 30)
                        message = '<epoch:{:3d}, model_idx: {:1d}, iter:{:8,d}, lr_0:{:.3e}, lr_1: {:.3e}, t:{:.2e}, td:{:.2e}, eta:{:.2e}, nll:{:.3e}, ram:{:.3f}> '.format(
                            epoch, train_idx, current_step, models[0].get_current_learning_rate(), models[1].get_current_learning_rate(), 
                            avg_time, avg_data_time, eta(avg_time), nll, ram)
                        print(message)
                    ...
                    if current_step % opt['logger']['save_checkpoint_freq'] == 0:
                        if rank <= 0:
                            ...
                            for train_idx_, model_ in models.items():
                                model_.save_network(model_.netG, f'G_{train_idx_}', current_step)
                                # model_.save_training_state(epoch, current_step)
                ...
                if ((current_step % opt['train']['val_freq'] == 0)
                    ...
                    for train_idx_, model_ in models.items():
                        ...
                        for idx, val_data in enumerate(val_loader):
                            ...
                            model_.feed_data(val_data)

                            nll, epses, y_label = model_.test()
                            if nll is None:
                                nll = 0
                            nlls.append(nll)
                            ...
                            if idx < n_visual:
                                ...
                                img_dir = os.path.join(opt['path']['val_images'], img_name, f"model_{train_idx_}")
                                util.mkdir(img_dir)

                                for y_label_target in labels:
                                    visuals = model_.get_current_visuals(y_label=[y_label_target])

                                    sr_img = None
                                    # Save SR images for reference
                                    if hasattr(model_, 'heats'):
                                        for heat in model_.heats:
                                            for i in range(model_.n_sample):
                        ...
                        # log
                        logger.info('# Validation # model_idx: {} PSNR: {:.4e} # SSIM: {:.4e} # LPIPS: {:.4e} '.format(train_idx_, avg_psnr, avg_ssim, avg_lpips))
                        ...
                        logger_val.info('<epoch:{:3d}, model_idx: {}, iter:{:8,d}> psnr: {:.4e} | ssim: {:.4e} | lpips {:.4e}'.format(
                            epoch, train_idx_, current_step, avg_psnr, avg_ssim, avg_lpips))
    ...
    if rank <= 0:
        ...
        models[0].save_network(models[0].netG, 'G_0', "latest")
        models[1].save_network(models[1].netG, 'G_1', "latest")
