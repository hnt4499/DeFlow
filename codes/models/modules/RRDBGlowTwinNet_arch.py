...
class RRDBGlowTwinNet(nn.Module):
    ...
    def get_flow_layer(self, flow_idx):
        if flow_idx == 0:
            return self.flowUpsamplerNet_0
        elif flow_idx == 1:
            return self.flowUpsamplerNet_1
        else:
            raise KeyError
    ...
    def normal_flow(self, gt, lr, y_onehot=None, epses=None, lr_enc=None, add_gt_noise=True, step=None, flow_idx: int = None):
        assert flow_idx in [0, 1], "`flow_idx` must be either 0 or 1"
        ...
        # Encode
        epses, logdet = self.get_flow_layer(flow_idx)(
            rrdbResults=lr_enc, gt=z, logdet=logdet, reverse=False, epses=epses, y_onehot=y_onehot
        )
